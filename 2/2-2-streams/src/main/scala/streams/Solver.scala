...
trait Solver extends GameDef {
   ...
   */
  def done(b: Block): Boolean = goal.col == b.b1.col && goal.col == b.b2.col && goal.row == b.b1.row && goal.row == b.b2.row
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    ...
    b.legalNeighbors.foldRight(LazyList() : LazyList[(Block, List[Move])])(
      ( bm: (Block, Move), acc: LazyList[(Block, List[Move])] )  => {
        acc.appended((bm._1, history.appended(bm._2 : Move) ))
      }
    )
  }
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filter( bm => !explored.contains(bm._1) )
    ...
    initial match {
      ...
      case x #:: xs => {
        val more : LazyList[(Block, List[Move])] = for {
          (b, m) <- initial
          next <- newNeighborsOnly(neighborsWithHistory(b, m), explored.incl(b))
        } yield next

        val newExplored : Set[Block] = more.foldLeft(explored)((acc: Set[Block], bm: (Block, List[Move])) =>{
          acc.incl(bm._1)
        })
        from(more: LazyList[(Block, List[Move])], newExplored: Set[Block]).foldRight(initial)( (bm: (Block, List[Move]), acc: LazyList[(Block, List[Move])]) =>{
          bm #:: acc
        })
      }
    }
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter(bm => done(bm._1))
