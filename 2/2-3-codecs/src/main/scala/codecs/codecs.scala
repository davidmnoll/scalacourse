...
import codecs.Decoder.stringDecoder
...
trait DecoderInstances {
  ...
  implicit def intDecoder: Decoder[Int] = Decoder.fromFunction ( data =>  {
    data match {
      case data: Json.Num => {
        if( data.value.isValidInt )
          Some(data.value.intValue : Int ) : Option[Int]
  ...
  implicit val booleanDecoder: Decoder[Boolean] =
    Decoder.fromPartialFunction { case b: Json.Bool => b.value.booleanValue() }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction { (listData: Json)  => { listData match {
      case y : Json.Arr => {  y.items.foldRight(Some(Nil): Option[List[A]])( ( data: Json, listoption: Option[List[A]]  ) => {
        listoption match {
          case Some(x) => { decoder.decode(data) match {
              case Some(y) =>  Some(y :: x) : Option[List[A]]
  ...
  /**
    ...
    */
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] = Decoder.fromFunction { (objData: Json)  => { objData match {
      case objData: Json.Obj => decoder.decode( objData.fields(name) )
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.fromFunction( (data:Json) => { data match {
      case objData: Json.Obj =>
        val name = Decoder.field[String]("name").decode(objData)
        val age = Decoder.field[Int]("age").decode(objData)
        (name, age) match {
          case (Some(n), Some(a)) => Some(Person(n, a))
          case _ => None
        }
    }})
...
trait ContactsCodecs {
  ...
  implicit val contactDecoder: Decoder[Contacts] = Decoder.fromFunction( contacts => contacts match {
    case obj: Json.Obj => {
      Decoder.field[List[Person]]("people").decode(obj) match {
        case Some(people) => Some(Contacts(people))
        case None => None
      }
    }
  } )
